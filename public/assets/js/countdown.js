...
function getNextYear(){
    // nếu tháng hiện tại < 2 thì trả về năm hiện tại
...
var animation = {
  ...
  get period() {
    ...
    var lunar = jdn2date(getYearInfo(nextYear)[0].jd);
    var dateFuture = new Date(lunar[2], lunar[1] - 1, lunar[0]);
  ...
  year: function (className) {
    ...
    for (
      var i = 0;
      i <= String(animation.period.year).length - 1;
      i++
    ) {
  ...
  animate: function () {
    ...
    var controls = animation.element(
      animation.newYear,
      "div",
      "controls"
    );
  ...
  plural: function (property) {
    ...
    return Number(period[property]) > 1
      ...
      : period[property] +
          " " +
          property.substr(0, property.length - 1);
  },
  render: function () {
...
animation.animate();
document.title = "Happy New Year " + getNextYear();
